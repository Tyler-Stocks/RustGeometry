...
impl AddAssign for Point2D<f64> {
...
impl AddAssign for Point2D<i128> {
    fn add_assign(&mut self, rhs: Self) {
        self.x = self.x + rhs.x;
        self.y = self.y + rhs.y;
    }
...
impl Reflect for Point2D<i128> {
    fn reflect_x(&mut self) -> () { self.x *= -1; }
    fn reflect_y(&mut self) -> () { self.y *= -1; }
}

impl Reflect for Point2D<f64> {
